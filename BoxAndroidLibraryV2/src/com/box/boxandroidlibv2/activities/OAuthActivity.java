...
/**
 * Activity for OAuth. Use this activity by using the intent from
 * createOAuthActivityIntent method. On completion, this activity will put the
 * parcelable BoxAndroidClient into activity result. In case of failing, the
 * activity result will be {@link Activity#RESULT_CANCELED} together will a
 * error message in intent extra.
 */
public class OAuthActivity extends Activity {
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		boolean allowShowRedirect = getIntent().getBooleanExtra(
				ALLOW_LOAD_REDIRECT_PAGE, true);
		startOAuth(clientId, clientSecret, redirectUrl, allowShowRedirect,
				deviceId, deviceName);
	}
	 ...
	 * @param deviceId
	 */
	protected void startOAuth(final String clientId, final String clientSecret,
			String redirectUrl, boolean allowShowRedirect, String deviceId,
			String deviceName) {
		BoxAndroidClient boxClient = new BoxAndroidClient(clientId,
		...
		if (StringUtils.isNotEmpty(deviceId)
				&& StringUtils.isNotEmpty(deviceName)) {
	...
	/**
	 * Create intent to launch OAuthActivity,use this method only if you already
	 * set redirect url in <a
	 * href="https://cloud.app.box.com/developers/services">box dev console</a>
	 * and you want to show the redirect page at the end of OAuth flow.
	 * 
	 * @param context
	 *            context
	 * @param clientId
	 ...
	 * @return
	 */
	public static Intent createOAuthActivityIntent(final Context context,
	...
	/**
	 * Create intent to launch OAuthActivity, use this method only if you
	 * already set redirect url in <a
	 * href="https://cloud.app.box.com/developers/services">box dev console</a>.
	 * 
	 * @param context
	 *            context
	 ...
	 *            Whether you want to load/show redirected page after OAuth flow
	 *            is done.
	 * @return
	 */
	public static Intent createOAuthActivityIntent(final Context context,
			final String clientId, final String clientSecret,
			final boolean allowShowRedirectPage) {
		return createOAuthActivityIntent(context, clientId, clientSecret,
				allowShowRedirectPage, null);
	}

	/**
	 * Create intent to launch OAuthActivity. Notes about redirect url
	 * parameter: If you already set redirect url in <a
	 * href="https://cloud.app.box.com/developers/services">box dev console</a>,
	 * you should pass in the same redirect url or use null for redirect url. If
	 * you didn't set it in box dev console, you should pass in a url. In case
	 * you don't have a redirect server you can simply use "http://localhost".
	 * 
	 * @param context
	 *            context
	 * @param clientId
	 ...
	 *            is done.
	 * @param redirectUrl
	 *            redirect url, if you already set redirect url in <a
	 *            href="https://cloud.app.box.com/developers/services">box dev
	 *            console</a>, leave this null or use the same url, otherwise
	 *            this field is required. You can use "http://localhost" if you
	 *            don't have a redirect server.
	 * @return
	 */
	public static Intent createOAuthActivityIntent(final Context context,
			final String clientId, final String clientSecret,
	...
	protected OAuthWebViewListener getOAuthFlowListener() {
		return new OAuthWebViewListener() {
			...
			@Override
			public void onAuthFlowEvent(final IAuthEvent event,
					final IAuthFlowMessage message) {
				if (event == OAuthEvent.OAUTH_CREATED) {
					Intent intent = new Intent();
					// intent.putExtra(BOX_CLIENT_OAUTH,
					// (BoxAndroidOAuthData) message.getData());
					Bundle b = new Bundle();
					b.putParcelable(BOX_CLIENT_OAUTH,
							(BoxAndroidOAuthData) message.getData());
					intent.putExtras(b);
