...
/**
 * This class is used to choose a particular file from the user's account and
 * when chosen will return a result with a BoxAndroidFile in the extra
...
public class FilePickerActivity extends FolderNavigationActivity {
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			mCurrentFolder = savedInstanceState
					.getParcelable(EXTRA_BOX_ANDROID_FOLDER);
	...
	protected void onResume() {
		...
		if (mCurrentFolder == null && mFolderName != null) {
			setNavigationSpinner(new NavigationItem(mFolderName,
					mCurrentFolderId));
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == FILE_PICKER_REQUEST_CODE) {
			if (resultCode == Activity.RESULT_OK) {
				// if someone further down the chain has chosen a file then send
				// result back.
	...
	private void openFolder(final String folderId) {
		if (folderId == null || mCurrentFolder != null
				&& mCurrentFolder.getId().equals(folderId)) {
			return;
		}
		try {
			Intent intent = getLaunchIntent(this, folderId,
	...
	private void openFolder(final String folderName, final String folderId) {
		...
		try {
			Intent intent = getLaunchIntent(this, folderName, folderId,
					(BoxAndroidOAuthData) mClient.getAuthData(), clientId,
					clientSecret);
			intent.setClass(this, getClass());
			intent.putExtra(EXTRA_NAV_NUMBER, (mNavNumber + 1));
	...
	protected void handleFileClick(final BoxAndroidFile file) {
		...
		Bundle b = new Bundle();
		b.putParcelable(EXTRA_BOX_ANDROID_FILE, file);
		intent.putExtras(b);
	...
	public static Intent getLaunchIntent(Context context,
		...
		Intent intent = FolderNavigationActivity.getLaunchIntent(context,
				folderId, oauth, clientId, clientSecret);
	...
	/**
	 * Create an intent to launch an instance of this activity to navigate
	 * folders. This version will immediately show the given name in the
	 * navigation spinner to before information about it has been fetched from
	 * the server.
	 * 
	 * @param context
	 *            current context.
	 * @param folderName
	 *            Name to show in the navigation spinner. Should be name of the
	 *            folder.
	 * @param folderId
	 *            folder id to navigate.
	 * @param oauth
	 ...
	 */
	public static Intent getLaunchIntent(Context context,
			final String folderName, final String folderId,
			final BoxAndroidOAuthData oauth, final String clientId,
			final String clientSecret) {
		Intent intent = getLaunchIntent(context, folderId, oauth, clientId,
				clientSecret);
	...
	protected void setNavigationSpinner(final BoxAndroidFolder currentFolder) {
		...
		ArrayList<NavigationItem> navigationFolders = new ArrayList<NavigationItem>();
		navigationFolders.add(new NavigationItem(currentFolder.getName(),
				currentFolder.getId()));
		for (int i = currentFolder.getPathCollection().getEntries().size() - 1; i >= 0; i--) {
			BoxItem item = (BoxItem) currentFolder.getPathCollection()
					.getEntries().get(i);
			navigationFolders.add(new NavigationItem(item.getName(), item
					.getId()));
		}
		Spinner spinner = getNavigationSpinner();
		spinner.setAdapter(new NavigationListAdapter(this, navigationFolders));
		spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
					int pos, long id) {
				NavigationItem item = (NavigationItem) parent
