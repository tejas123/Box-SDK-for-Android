 ...
 * 
 */
public class FolderNavigationActivity extends Activity implements
		OnItemClickListener {
	...
	/**
	 * Because this activity will launch instances of itself this indicates how
	 * deep the activity stack is for these internal activities.
	 */
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		if (getIntent() != null) {
			...
			BoxAndroidOAuthData authData = getIntent().getParcelableExtra(
		...
		if (savedInstanceState != null) {
			...
			clientSecret = savedInstanceState
					.getString(EXTRA_BOX_CLIENT_SECRET);
			BoxAndroidOAuthData authData = savedInstanceState
					.getParcelable(EXTRA_BOX_CLIENT_OAUTH);
			mClient = new BoxAndroidClient(clientId, clientSecret, null, null,
					(new BoxAndroidConfigBuilder()).build());
			mClient.authenticate(authData);
		...
		if (mClient == null) {
			if (mClient == null) {
				Toast.makeText(this, "No client provided", Toast.LENGTH_LONG)
						.show();
				finish();
				return;
			}
		...
		((BoxListItemAdapter) mListView.getAdapter()).add(new BoxListItem(
				mController.fetchFolder(mCurrentFolderId),
				Controller.ACTION_FETCHED_FOLDER));
		mThumbnailManager.deleteFilesInCacheDirectory();
	}

	/**
	 * 
	 * @return The intent filter used to listen to broadcasts for folder
	 *         navigation.
	 */
	 ...
	 *            The thumbnail manager provided by
	 *            initializeThumbnailManager().
	 * @return BoxListItemAdapter to use for showing files or folders.
	 */
	protected BoxListItemAdapter initializeBoxListItemAdapter(
	...
	/**
	 * 
	 * @return Thumbnail manager to be used for thumbnails for this navigation
	 *         actiivty.
	 */
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		try {
			outState.putParcelable(EXTRA_BOX_CLIENT_OAUTH,
					(BoxAndroidOAuthData) mClient.getAuthData());
	...
	/**
	 * Create an intent to launch an instance of this activity to navigate
	 * folders.
	 * 
	 * @param context
	 *            current context.
	...
	public static Intent getLaunchIntent(Context context,
			final String folderId, final BoxAndroidOAuthData oauth,
			final String clientId, final String clientSecret) {
		Intent intent = new Intent(context, FolderNavigationActivity.class);
		Bundle b = new Bundle();
		b.putString(EXTRA_FOLDER_ID, folderId);
		b.putParcelable(EXTRA_BOX_CLIENT_OAUTH, oauth);
		b.putString(EXTRA_BOX_CLIENT_ID, clientId);
		b.putString(EXTRA_BOX_CLIENT_SECRET, clientSecret);
		intent.putExtras(b);
		return intent;
	}
		...
		if (!intent.getBooleanExtra(Controller.ARG_SUCCESS, false)) {
			Toast.makeText(
					...
					getResources().getString(
							R.string.boxandroidlibv2_Problem_fetching_folder),
					Toast.LENGTH_LONG).show();
			return;
		}

		if (mCurrentFolderId.equals(intent
				.getStringExtra(Controller.ARG_FOLDER_ID))) {

			BoxAndroidFolder boxFolder = intent
					.getParcelableExtra(Controller.ARG_BOX_FOLDER);
			final BoxAndroidCollection collection = boxFolder
					.getItemCollection();

			BoxListItemAdapter adapter = ((BoxListItemAdapter) mListView
					.getAdapter());
			adapter.remove(intent.getAction());
			if (collection.getTotalCount() > 0) {
				// because the default folder items don't have all the fields we
				// need do folder item fetch if there are any additional items.
				((BoxListItemAdapter) mListView.getAdapter())
						.add(new BoxListItem(mController.fetchFolderItems(
								mCurrentFolderId, 0, collection.getEntries()
										.size()),
	...
	protected void onFetchedFolderItems(final Intent intent) {
		...
		if (mCurrentFolderId.equals(intent
				.getStringExtra(Controller.ARG_FOLDER_ID))) {
			BoxAndroidCollection collection = intent
					.getParcelableExtra(Controller.ARG_BOX_COLLECTION);
			BoxListItemAdapter adapter = ((BoxListItemAdapter) mListView
					.getAdapter());
			...
			if (offset + collection.getEntries().size() < collection
					.getTotalCount()) {
				// if not all entries were fetched add a task to fetch more
				// items if user scrolls to last entry.
				((BoxListItemAdapter) mListView.getAdapter())
						.add(new BoxListItem(
								mController
										.fetchFolderItems(mCurrentFolderId,
												offset
														+ collection
																.getEntries()
																.size(), limit),
	...
	protected void onDownloadedThumbnail(final Intent intent) {
		if (intent.getBooleanExtra(Controller.ARG_SUCCESS, false)) {
			((BoxListItemAdapter) mListView.getAdapter()).update(intent
					.getStringExtra(Controller.ARG_FILE_ID));
		}
	}
	...
	protected ThreadPoolExecutor getApiExecutor() {
		if (apiExecutor == null || apiExecutor.isShutdown()) {
			apiExecutor = new ThreadPoolExecutor(2, 10, 3600, TimeUnit.SECONDS,
	...
	protected ThreadPoolExecutor getThumbnailApiExecutor() {
		if (thumbnailApiExecutor == null || thumbnailApiExecutor.isShutdown()) {
			thumbnailApiExecutor = new ThreadPoolExecutor(1, 10, 3600,
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(Controller.ACTION_FETCHED_FOLDER)) {
				...
				onFetchedFolderItems(intent);
					...
					Controller.ACTION_DOWNLOADED_FILE_THUMBNAIL)) {
	 ...
	 * 
	 */
	protected class FolderNavigationBoxListItemAdapter extends
			BoxListItemAdapter {

		public FolderNavigationBoxListItemAdapter(Activity context,
		...
		protected void update(ViewHolder holder, BoxListItem listItem) {
			...
			if (listItem.getType() == BoxListItem.TYPE_FUTURE_TASK) {
				...
				return;
			} else if (listItem.getType() == BoxListItem.TYPE_BOX_FILE_ITEM
					|| (listItem.getType() == BoxListItem.TYPE_BOX_FOLDER_ITEM)) {

				if (listItem.getBoxItem() instanceof BoxAndroidFile) {
					if (listItem.getTask() == null) {
						listItem.setTask(mController.downloadThumbnail(listItem
								.getBoxItem().getId(), mThumbnailManager
								.getThumbnailForFile(listItem.getBoxItem()
										.getId()), holder));
					} else if (listItem.getTask().isDone()) {
						try {
							Intent intent = (Intent) listItem.getTask().get();
							// if we were unable to get this thumbnail before
							// try it again.
							if (!intent.getBooleanExtra(Controller.ARG_SUCCESS,
									false)) {
								listItem.setTask(mController.downloadThumbnail(
										listItem.getBoxItem().getId(),
										mThumbnailManager
												.getThumbnailForFile(listItem
														.getBoxItem().getId()),
										holder));
							}
						} catch (Exception e) {
	...
	public class Controller {
		 ...
		 * @return A FutureTask that is tasked with fetching information on the
		 *         given folder.
		 */
			...
			return new FutureTask<Intent>(new Callable<Intent>() {
				...
				public Intent call() throws Exception {
					...
					try {
						...
						defaultRequest.getRequestExtras().addQueryParam(
								EXTRA_NAV_NUMBER, Integer.toString(mNavNumber));
						defaultRequest.getRequestExtras().addQueryParam(
								EXTRA_SOURCE_TYPE, getSourceType());
						BoxAndroidFolder bf = (BoxAndroidFolder) mClient
								.getFoldersManager().getFolder(folderId,
										defaultRequest);
						if (bf != null) {
							// intent.putExtra(ARG_SUCCESS, true);
							b.putBoolean(ARG_SUCCESS, true);
							// intent.putExtra(Controller.ARG_BOX_FOLDER, bf);
							b.putParcelable(Controller.ARG_BOX_FOLDER, bf);
						}
						...
						b.putBoolean(ARG_SUCCESS, false);
		 ...
		 *         given folder.
		 */
		public FutureTask<Intent> fetchFolderItems(final String folderId,
			...
			return new FutureTask<Intent>(new Callable<Intent>() {
				...
				public Intent call() throws Exception {
					...
					b.putInt(ARG_OFFSET, offset);
					// intent.putExtra(ARG_LIMIT, limit);
					b.putInt(ARG_LIMIT, limit);
					// intent.putExtra(ARG_FOLDER_ID, folderId);
					b.putString(ARG_FOLDER_ID, folderId);
					try {

						// this call the collection is just BoxObjectItems and
						// each does not appear to be an instance of BoxItem.
						...
						BoxPagingRequestObject obj = BoxPagingRequestObject
								.pagingRequestObject(limit, offset);
						obj.getRequestExtras().addFields(itemFields);
						BoxAndroidCollection bc = (BoxAndroidCollection) mClient
								.getFoldersManager().getFolderItems(folderId,
										obj);
						if (bc != null) {
							// intent.putExtra(ARG_SUCCESS, true);
							b.putBoolean(ARG_SUCCESS, true);
							// intent.putExtra(Controller.ARG_BOX_COLLECTION,
							// bc);
							b.putParcelable(Controller.ARG_BOX_COLLECTION, bc);
						}
		 ...
		 *         given folder.
		 */
		public FutureTask<Intent> downloadThumbnail(final String fileId,
				final File downloadLocation, final ViewHolder holder) {
			return new FutureTask<Intent>(new Callable<Intent>() {
				...
				public Intent call() throws Exception {
					...
					intent.putExtra(ARG_SUCCESS, false);
					try {
						// no need to continue downloading thumbnail if we
						...
						// no need to continue downloading thumbnail if we are
						// not viewing this thumbnail.
						if (holder.getBoxListItem() == null
								|| !(holder.getBoxListItem().getBoxItem() instanceof BoxAndroidFile)
								|| !holder.getBoxListItem().getBoxItem()
										.getId().equals(fileId)) {
							intent.putExtra(ARG_SUCCESS, false);
							return intent;
						}
						// this call the collection is just BoxObjectItems and
						// each does not appear to be an instance of BoxItem.
						InputStream input = mClient
								.getFilesManager()
								.getThumbnail(
										fileId,
										"png",
										BoxImageRequestObject
												.previewRequestObject())
								.getContent();
	...
	public void onItemClick(AdapterView<?> listView, View v, int position,
			long id) {
		BoxListItem listItem = (BoxListItem) listView
	...
	protected void handleFolderClick(BoxAndroidFolder folder) {
		try {
			Intent intent = getLaunchIntent(this, folder.getId(),
					(BoxAndroidOAuthData) mClient.getAuthData(), clientId,
					clientSecret);
	...
	/**
	 * Handle response if we are unable to authenticate our api calls with the
	 * given client.
	 */
