...
public class OAuthWebView extends WebView implements IAuthFlowUI {
	...
	@Override
	public void initializeAuthFlow(final Object activity,
			final String clientId, final String clientSecret) {
		initializeAuthFlow(activity, clientId, clientSecret, null);
	}

	@Override
	public void initializeAuthFlow(Object activity, String clientId,
		...
		initializeAuthFlow(
				...
				clientSecret,
				redirectUrl,
				new BoxAndroidClient(clientId, clientSecret, hub,
						new BoxJSONParser(hub), (new BoxAndroidConfigBuilder())
								.build()));
	}
	...
	@Override
	public void initializeAuthFlow(Object activity, String clientId,
			String clientSecret, String redirectUrl, BoxClient boxClient) {
		this.mWebViewData = new OAuthWebViewData(
				boxClient.getOAuthDataController());
	...
	private static OAuthWebViewListener wrapOAuthWebViewListener(
	...
	protected OAuthWebViewClient createOAuthWebViewClient(
			OAuthWebViewData data, Object activity, BoxClient boxClient) {
		OAuthWebViewClient c = new OAuthWebViewClient(data,
				(Activity) activity, boxClient);
	...
	public static class OAuthWebViewClient extends WebViewClient {
		 ...
		 */
		public OAuthWebViewClient(final OAuthWebViewData webViewData,
				final Activity activity, final BoxClient boxClient) {
			super();
		...
		public void onPageStarted(final WebView view, final String url,
			...
			for (IAuthFlowListener listener : mListeners) {
				if (listener != null) {
					listener.onAuthFlowEvent(OAuthEvent.PAGE_STARTED,
							new StringMessage(StringMessage.MESSAGE_URL, url));
				}
			}
			...
			if (StringUtils.isNotEmpty(code)) {
				for (IAuthFlowListener listener : mListeners) {
					if (listener != null) {
						listener.onAuthFlowMessage(new StringMessage(
								mwebViewData.getResponseType(), code));
					}
				}
		...
		public void onReceivedHttpAuthRequest(final WebView view,
				...
				final String realm) {
			for (IAuthFlowListener listener : mListeners) {
				listener.onAuthFlowEvent(OAuthEvent.AUTH_REQUEST_RECEIVED,
						new StringMessage(host, realm));
			}
			LayoutInflater factory = mActivity.getLayoutInflater();
			final View textEntryView = factory.inflate(
					R.layout.boxandroidlibv2_alert_dialog_text_entry, null);

			AlertDialog loginAlert = new AlertDialog.Builder(mActivity)
					.setTitle(R.string.boxandroidlibv2_alert_dialog_text_entry)
					.setView(textEntryView)
					.setPositiveButton(
							R.string.boxandroidlibv2_alert_dialog_ok,
					...
					.setNegativeButton(
							R.string.boxandroidlibv2_alert_dialog_cancel,
							new DialogInterface.OnClickListener() {
		...
		public void onPageFinished(final WebView view, final String url) {
			fireEvents(OAuthEvent.PAGE_FINISHED, new StringMessage(
					StringMessage.MESSAGE_URL, url));
		}
		...
		private void startMakingOAuthAPICall(final String code, WebView view) {
			...
			try {
				dialog = ProgressDialog
						.show(mActivity,
								mActivity
										.getText(R.string.boxandroidlibv2_Authenticating),
								mActivity
			...
			AsyncTask<Null, Null, BoxAndroidOAuthData> task = new AsyncTask<Null, Null, BoxAndroidOAuthData>() {
				...
				@Override
				protected BoxAndroidOAuthData doInBackground(
						final Null... params) {
					BoxAndroidOAuthData oauth = null;
					try {
						oauth = (BoxAndroidOAuthData) mBoxClient
								.getOAuthManager().createOAuth(code,
										mwebViewData.getClientId(),
										mwebViewData.getClientSecret(),
				...
				protected void onPostExecute(final BoxAndroidOAuthData result) {
					...
					if (result != null) {
						try {
							fireEvents(
									OAuthEvent.OAUTH_CREATED,
									new OAuthDataMessage(result, mBoxClient
											.getJSONParser(), mBoxClient
											.getResourceHub()));
						} catch (Exception e) {
		...
		public void onReceivedError(final WebView view, final int errorCode,
			...
			if (!allowShowRedirectPage()
					&& oauthAPICallState != OAuthAPICallState.PRE) {
				// Error happens after oauth api call started. This can only be
				// the redirect page. In case user wants to ignore redirect
				// page, we
				// swallow this error.
				return;
			}
		...
		public void onReceivedSslError(final WebView view,
			...
			StringBuilder sslErrorMessage = new StringBuilder(
					resources
							.getString(R.string.boxandroidlibv2_There_are_problems_with_the_security_certificate_for_this_site));
			...
			case SslError.SSL_DATE_INVALID:
				sslErrorType = view
						.getResources()
						.getString(
								R.string.boxandroidlibv2_ssl_error_warning_DATE_INVALID);
				break;
			case SslError.SSL_EXPIRED:
				sslErrorType = resources
						.getString(R.string.boxandroidlibv2_ssl_error_warning_EXPIRED);
				break;
			case SslError.SSL_IDMISMATCH:
				sslErrorType = resources
						.getString(R.string.boxandroidlibv2_ssl_error_warning_ID_MISMATCH);
				break;
			case SslError.SSL_NOTYETVALID:
				sslErrorType = resources
						.getString(R.string.boxandroidlibv2_ssl_error_warning_NOT_YET_VALID);
				break;
			case SslError.SSL_UNTRUSTED:
				sslErrorType = resources
						.getString(R.string.boxandroidlibv2_ssl_error_warning_UNTRUSTED);
				break;
			case SslError.SSL_INVALID:
				sslErrorType = resources
						.getString(R.string.boxandroidlibv2_ssl_error_warning_INVALID);
				break;
			default:
				sslErrorType = resources
						.getString(R.string.boxandroidlibv2_ssl_error_warning_INVALID);
			...
			sslErrorMessage
					.append(resources
							.getString(R.string.boxandroidlibv2_ssl_should_not_proceed));
			// Show the user a dialog to force them to accept or decline the SSL
			// problem before continuing.
			sslErrorDialogButtonClicked = false;
			AlertDialog loginAlert = new AlertDialog.Builder(view.getContext())
					.setTitle(R.string.boxandroidlibv2_Security_Warning)
					.setMessage(sslErrorMessage.toString())
					.setIcon(R.drawable.boxandroidlibv2_dialog_warning)
					.setPositiveButton(R.string.boxandroidlibv2_Continue,
							new DialogInterface.OnClickListener() {
					...
					.setNegativeButton(R.string.boxandroidlibv2_Go_back,
							new DialogInterface.OnClickListener() {
		...
		protected void handleReceivedError(final WebView view,
		...
		private void sendoutSslError(final SslError error,
	...
	private static class WrappedOAuthWebViewListener extends
